...
@app.callback([Output(component_id='dropdown', component_property= 'options'),
              Output(component_id='dropdown', component_property= 'value')],
              [Input(component_id='radioitems', component_property= 'value')])


def dropdown_options(radioitems_value):
    
    if radioitems_value == 'Distance_covered_gear_wise':
        options =0
        value = 0
    if radioitems_value=='Mileage':
        options = options = [
            {'label':'For_All_Mileage', 'value':'For_All_Mileage' },
           
            ]
        value = 'For_All_Mileage'
    else:
        options = options = [
...
def select_data(dropdown_value,radioitems_value):
    ...
    if radioitems_value=='Mileage' :
        if dropdown_value=='For_All_Mileage':
            print('sssss',dropdown_value,radioitems_value)
            dfg= pd.read_csv(r'C:\Users\SARATHH\internal_data_manupulation (1)\webpage_flask\web_app\data\time_gap_6hourss.csv')
            #dfg.reset_index(drop=True, inplace=True)
            dfg = dfg.set_index(['ind']).sort_index(level=[0,1])
            
            trace1 = go.Bar(
                x=dfg.index.get_level_values('ind'),
                y=dfg.ALT,
                name='Altitude',
                marker=dict(
                    color='rgb(0,0,0)'
            ...
            fig.update_layout(
                    title='Altitude across mileage',
                    xaxis_title="Index"  ,
                    yaxis_title="Altitude_Range")
